...
| 049 | Play through and validate all levels | Open   | Systematically play through every level in the game to identify and fix misconfigured levels. Check for: incorrect/impossible validation logic, unclear descriptions, missing or wrong input/output configurations, difficulty spikes, typos in level text. Document any issues found and fix them. Ensure all levels are completable, well-explained, and provide appropriate difficulty progression. Quality assurance pass for the entire level system. |
| 050 | Fix chip navigation in ChipLibraryMenu | Open   | Fix navigation issues in the Chip Library Menu (ChipLibraryMenu.cs). Current issues may include: incorrect keyboard/gamepad navigation between chips and collections, selection state not updating properly when navigating, visual feedback not matching actual selection. Investigate navigation flow and ensure intuitive movement between starred items, collections, nested collections, and chips. Ensure proper focus management and selection highlighting. Mobile and PC compatibility required. |
